  ...
  return (
    ...
    <header className="cs-header" style={{
      ...
      <div className="cs-header-inner" style={{
        ...
        <a href={homeUrl()} className="cs-brand" aria-label="Чудо Садик — на главную"
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }} className="cs-nav">
          ...
          <a href={'tel:' + CFG.PHONE_PRIMARY} aria-label="Позвонить: +998 94 671 26 26" title="Позвонить"
            onClick={() => goal('click_phone')}
            style={{
              width: 48, height: 48, borderRadius: 'var(--radius-pill)',
              display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
              background: '#fff', border: '2px solid var(--pink-200, #FFC2DA)',
              color: 'var(--pink-500, #FF4D97)', textDecoration: 'none',
      ...
      <nav className={`cs-mobile-nav${menuOpen ? ' cs-mobile-nav-open' : ''}`} aria-label="Мобильное меню" style={{
        ...
        <a href={'tel:' + CFG.PHONE_PRIMARY} onClick={() => goal('click_phone')}
          style={{
            padding: '12px 14px', borderRadius: 'var(--radius-md)',
            fontWeight: 800, fontSize: '16px', color: 'var(--color-primary)', textDecoration: 'none',
            display: 'flex', alignItems: 'center', gap: '8px',
          }}>
          <Ic n="phone" size={18} /> +998 94 671 26 26
        </a>
