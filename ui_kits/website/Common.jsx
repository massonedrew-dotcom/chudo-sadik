...
const PHOTO_SIZES = {
  ...
  'crafts': [800, 1200], 'games': [800, 1200], 'cta-phone': [675, 1200],
  'dance': [899, 1200], 'family': [899, 1200], 'gymnastics': [899, 1200],
  'kids-reading': [899, 1200], 'massage': [899, 1200], 'playroom': [899, 1200],
  'robotics': [899, 1200], 'summer-pool': [899, 1200], 'taekwondo': [899, 1200],
  'map-route': [1200, 1016],
  'banner-welcome-1200': [1200, 439], 'banner-welcome-1800': [1800, 658],
  'banner-welcome-2600': [2600, 951],
};
...
/* Знак-солнышко. Отдельный файл на 256 px: исходный логотип весит 666 КБ и
   грузился на каждой странице ради картинки размером с ноготь. */
function SunMark({ size = 40, alt = 'Логотип Чудо Садик', className, style = {} }) {
  return <img src={asset('assets/logo-sun-256.png')} alt={alt} width={size} height={size}
...
async function sendLead(payload) {
  ...
  if (!CFG.FORM_ENDPOINT) {
    console.warn('[Чудо Садик] FORM_ENDPOINT не задан в site.config.js — заявка НЕ отправлена:', data);
  ...
  try {
    ...
    console.error('[Чудо Садик] Не удалось отправить заявку:', err);
...
function ConsentCheck({ checked, onChange, id }) {
  return (
    <label htmlFor={id} style={{
      ...
      <span>
        Согласен(на) на{' '}
        <a href={asset('consent.html')} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-primary)' }}>
          обработку персональных данных
        </a>{' '}и с{' '}
        <a href={asset('privacy.html')} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-primary)' }}>
